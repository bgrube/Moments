...
def setCbFriendlyStyle(
  ...
  opacity:       float = 1.0,  # opacity of markers and lines
) -> None:
  ...
  graphOrHist.SetLineColorAlpha(color, opacity)
  if setMarker:
    graphOrHist.SetMarkerColorAlpha(color, opacity)
...
def plotMoments(
  ...
  for momentPart, legendEntrySuffix in (("Re", "Real Part"), ("Im", "Imag Part")):  # plot real and imaginary parts separately
    histStack = ROOT.THStack(f"{outFileNamePrefix}compare_{momentLabel}_{momentPart}",
                             f"{histTitle};{xAxisTitle};" + ("Normalized" if normalizedMoments else "Unnormalized") + " Moment Value" + yAxisUnit)
...
def plotMomentsBootstrapDistributions1D(
  ...
  for qnIndex in HData.indices.qnIndices:
    ...
    for momentPart, legendEntrySuffix in (("Re", "Real Part"), ("Im", "Imag Part")):  # plot real and imaginary parts separately
      ...
      legend.Draw()
      canv.SaveAs(f"{histBs.GetName()}.{outFileType}")
...
def plotMomentsBootstrapDiff(
  ...
  for graph in (graphMomentValDiff, graphMomentUncertDiff):
    ...
    legend.Draw()
    canv.SaveAs(f"{graph.GetName()}.{outFileType}")
