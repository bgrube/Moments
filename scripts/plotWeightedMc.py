...
def plotWeightedMc(
  cfg:                       AnalysisConfig,
  useIntensityTerms:         MomentResult.IntensityTermsType,          # type of intensity terms used for intensity weights
  massBinning:               HistAxisBinning,                          # binning for subsystem mass axis
  weightedDataDirPathSuffix: str = "",                                 # suffix to append to the weighted data directory path
  nmbBinsAzim:               int                              = 72,    # number of bins for azimuthal variables
  nmbBinsOther:              int                              = 100,   # number of bins for other variables
  additionalColumnDefs:      dict[str, dict[str, str]] | None = None,  # additional columns to define
  ...
  """Overlays weighted accepted phase-space MC data generated by the
  function in `weightDataWithMoments.py` with real data for
  comparison"""
